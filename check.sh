...
if [ "$FIX_MODE" = true ]; then
    echo -e "${BLUE}║      MMM-WebcamImage Quality Checks (FIX MODE)        ║${NC}"
else
    echo -e "${BLUE}║          MMM-WebcamImage Quality Checks               ║${NC}"
fi
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
CONSOLE_LOG_COUNT=$(grep -r "console\.log" --include="*.js" --exclude-dir=node_modules . 2>/dev/null | wc -l)
...
# 11. Check Git status (if in a git repo)
if [ -d ".git" ]; then
